...
__global__
void gpu_add_block_sums_new(unsigned int* const d_out,
	const unsigned int* const d_in,
	unsigned int* const d_block_sums,
	const size_t numElems)
{
	unsigned int d_block_sum_val = 0;
	if(blockIdx.x > 0)
		d_block_sum_val = d_block_sums[blockIdx.x-1];
...
__global__
void gpu_sum_scan_new(unsigned int* const d_out,
	const unsigned int* const d_in,
	unsigned int* const d_block_sums,
	const size_t numElems)
{
	...
	unsigned int f_ind = threadIdx.x+1, s_ind = threadIdx.x + blockDim.x+1;
	s_out[threadIdx.x] = 0;
	s_out[threadIdx.x + blockDim.x] = 0;
	...
	// Downsweep step
	unsigned f_temp = f_ind, s_temp = s_ind;
	for (int s = 1; s < max_steps; s++)
	{
		f_ind -= f_ind & (-f_ind);
		s_ind -= s_ind & (-s_ind);
		if((f_ind !=0) && (f_ind == (f_ind & (-f_ind))))
			s_out[f_temp-1] = s_out[f_temp-1] + s_out[f_temp-(f_temp & (-f_temp))-1];
		if((s_ind !=0) && (s_ind == (s_ind & (-s_ind))))
			s_out[s_temp-1] = s_out[s_temp-1] + s_out[s_temp-(s_temp & (-s_temp))-1];
	...
	const size_t numElems)
{
	...
	// Sum scan data allocated to each block
	gpu_sum_scan_new<<<grid_sz, block_sz, sizeof(unsigned int) * max_elems_per_block >>>(d_out, d_in, d_block_sums, numElems);
	...
	if (grid_sz <= max_elems_per_block)
	{
		...
		checkCudaErrors(cudaMalloc(&d_dummy_blocks_sums, sizeof(unsigned int)));
		checkCudaErrors(cudaMemset(d_dummy_blocks_sums, 0, sizeof(unsigned int)));
		gpu_sum_scan_new<<<1, block_sz, sizeof(unsigned int) * max_elems_per_block>>>(d_block_sums, d_block_sums, d_dummy_blocks_sums, grid_sz);
	...
	else
	{
		...
		checkCudaErrors(cudaMalloc(&d_in_block_sums, sizeof(unsigned int) * grid_sz));
		checkCudaErrors(cudaMemcpy(d_in_block_sums, d_block_sums, sizeof(unsigned int) * grid_sz, cudaMemcpyDeviceToDevice));
		sum_scan_optim(d_block_sums, d_in_block_sums, grid_sz);
		checkCudaErrors(cudaFree(d_in_block_sums));
	}
	
	gpu_add_block_sums_new<<<grid_sz, block_sz>>>(d_out, d_out, d_block_sums, numElems);
